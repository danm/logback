...
const sqsOutput = function(QueueUrl, outputs) {
            ...
            if (typeof loc === 'object') {
                ...
                if (loc.sqs !== undefined) {
                    if (loc.sqs.level === undefined || loc.sqs.queueUrl === undefined || loc.sqs.region === undefined) {
                        ...
                        this.QueueUrl = loc.sqs.queueUrl;
                        ...
                        if (loc.sqs.accessKeyId !== undefined) { AWS.config.update({ accessKeyId: loc.sqs.accessKeyId }); }
                        if (loc.sqs.secretAccessKey !== undefined) { AWS.config.update({ secretAccessKey: loc.sqs.secretAccessKey }); }

                        sqs = new AWS.SQS({ apiVersion: '2012-11-05', endpoint: 'https://sqs.' + loc.sqs.region + '.amazonaws.com' });
                    }
                }
            }
        } else {
        ...
        if (typeof this.loc === 'string') {
            ...
            if (this.loc.sqs !== undefined) {
                if (rights(this.loc.sqs.level, type) === true) {
                    sqsOutput(this.QueueUrl, outputs);
                }
            }
